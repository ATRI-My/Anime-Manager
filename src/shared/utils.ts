 ...
 * - 正则表达式分割搜索词，处理多个连续空格
 */
export function fuzzySearch(query: string, animeList: Anime[]): Anime[] {
  ...
  if (searchTerms.length === 1) {
    ...
    return animeList.filter(anime => {
      ...
      if (anime.tags.some(tag => tag.toLowerCase().includes(term))) return true
      if (anime.description?.toLowerCase().includes(term)) return true
      return false
    })
  }
