...
exports.validateAnimeList = validateAnimeList;
function validateAnime(anime) {
    ...
    if (!Array.isArray(anime.episodes)) {
        ...
        // 验证每个episode
        var episodeErrors = [];
        var episodeNumbers = new Set();
        for (var _i = 0, _a = anime.episodes; _i < _a.length; _i++) {
            var episode = _a[_i];
            var episodeResult = validateEpisode(episode);
            if (!episodeResult.isValid) {
                episodeErrors.push.apply(episodeErrors, episodeResult.errors);
            }
            // 检查剧集编号重复
            if (episode.number !== undefined) {
                if (episodeNumbers.has(episode.number)) {
                    episodeErrors.push("\u5267\u96C6\u7F16\u53F7 ".concat(episode.number, " \u91CD\u590D"));
                }
...
function validateEpisode(episode) {
    ...
    if (!episode.url || typeof episode.url !== 'string' || episode.url.trim() === '') {
        ...
        var urlStr = episode.url.trim();
        // 检查是否是本地文件路径
        var isLocalFile = urlStr.startsWith('file://') ||
...
// 辅助函数：验证Anime数组
function validateAnimeList(animeList) {
    var errors = [];
    var animeIds = new Set();
    for (var _i = 0, animeList_1 = animeList; _i < animeList_1.length; _i++) {
        var anime = animeList_1[_i];
        var result = validateAnime(anime);
        if (!result.isValid) {
            errors.push("Anime \"".concat(anime.title || '未知', "\" \u9A8C\u8BC1\u5931\u8D25: ").concat(result.errors.join(', ')));
        }
        // 检查ID重复
        if (anime.id) {
            if (animeIds.has(anime.id)) {
                errors.push("Anime ID \"".concat(anime.id, "\" \u91CD\u590D"));
            }
